<template>
  <div class="wrapper">
    <div class="header-2">
      <h2>Your daily dose of Book of Mormon:</h2>
    </div>
    <div class="s-wrapper">
...
<script>
import json from "../assets/bom.json"
export default {
  name: "bomquote",
  data() {
    return {
    ...
    methods: {
      newVerse() {
        ...
        for (const chapter in this.scriptures.books[randomBook].chapters) { // eslint-disable-line no-unused-vars
        ...
        for (const verse in this.scriptures.books[randomBook].chapters[randomChapter].verses) { // eslint-disable-line no-unused-vars
          versRange = versRange + 1;
        }
        randomVerse = Math.floor(Math.random() * (versRange + 1));

        //Test
        console.log(this.scriptures.books[randomBook].book);
        console.log(this.scriptures.books[randomBook].chapters.length);
        console.log(randomVerse);
        console.log(this.scriptures.books[randomBook].chapters[randomChapter].verses[randomVerse].text);

        this.dailyVerse.bookName = this.scriptures.books[randomBook].book;
        this.dailyVerse.book = randomBook;
        this.dailyVerse.chapter = randomChapter;
        this.dailyVerse.verse = randomVerse;
        this.dailyVerse.text = this.scriptures.books[randomBook].chapters[randomChapter].verses[randomVerse].text;
